...
function nativeReport(tuple, overrides = {}) {
  const bound = tuple.bindSaariExactTupleIdentity(identity());
  const fields = {
    number: "7",
    repository: SUITE,
    type: "pull_request",
...
function producerBundle(modules, report) {
...
policy = {"reviewers": {"clawsweeper": "saari-clawsweeper"}}
...
export async function proveSuppliedConsumer(checkout, pin = loadConsumerPin()) {
  ...
  const admitted = parseWithSuppliedConsumer(
    checkout,
    producerBundle(modules, nativeReport(modules.tuple)),
  );
  ...
  const zip = producerBundle(modules, nativeReport(modules.tuple));
  const rejects = [
    ["repository mismatch", { repository: "dinkuskit/blocks" }],
  ...
  rejects.push({
    ...
    ...expectReject(() =>
      parseWithSuppliedConsumer(
        checkout,
        producerBundle(modules, nativeReport(modules.tuple, { review_scope: "P0-only" })),
      ),
    ),
  });
  rejects.push({
    ...
    ...expectReject(() =>
      parseWithSuppliedConsumer(
        checkout,
        producerBundle(modules, nativeReport(modules.tuple, { reviewer_actor: "clawsweeper" })),
      ),
    ),
  });
  rejects.push({
    ...
    ...expectReject(() =>
      parseWithSuppliedConsumer(
        checkout,
        producerBundle(modules, nativeReport(modules.tuple, { review_status: "failed" })),
      ),
    ),
  });
