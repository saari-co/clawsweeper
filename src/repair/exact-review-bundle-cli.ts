...
if (command === "create") {
  ...
  if (actionLedgerRoot) {
    throw new Error("Saari exact-tuple bundles reject action-ledger side files");
  }
...
function contextFromEnv(env: NodeJS.ProcessEnv): ExactReviewBundleContext {
  ...
  return {
    repository:
      optionalEnv(env, "EXACT_REVIEW_WORKFLOW_REPOSITORY") || requiredEnv(env, "GITHUB_REPOSITORY"),
