...
export const EXACT_REVIEW_BUNDLE_MAX_FILES = 1;
export const EXACT_REVIEW_BUNDLE_MAX_FILE_BYTES = 512 * 1024;
export const EXACT_REVIEW_BUNDLE_MAX_TOTAL_BYTES = 1024 * 1024;
...
const FILE_PATH_PATTERN = /^review\/[1-9]\d*\.md$/;
...
export function createExactReviewBundle(
  ...
  if (files.length !== (artifactPresent ? 1 : 0)) {
    throw new Error("exact review bundle must contain only the review report");
  }
